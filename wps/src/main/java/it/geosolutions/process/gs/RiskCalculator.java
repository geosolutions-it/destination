...
public class RiskCalculator extends RiskCalculatorBase {
    ...
    public SimpleFeatureCollection execute(
        ...
        if (processing == 3) {
            ...
            if (changedTargets != null && !changedTargets.equals("")) {
                try {
                    loadSimulationTargets(dataStore, simulationTargets,
                            changedTargets);
    ...
    private void checkProcessingAllowed(JDBCDataStore dataStore, String dataStoreName, int processing)
        ...
        FeatureCollection fc = source.getFeatures(ff2.equals(
                ff2.property("id_elaborazione"), ff2.literal(processing)));
        // check if the given processing is allowed (if not is filtered out from
        // results
        // so size should be 0)
        if (fc.size() != 1) {
     ...
     * @throws IOException
     */
    public static void loadSimulationTargets(JDBCDataStore dataStore,
            List<TargetInfo> valuesMap, String valuesList) throws ParseException,
     ...
     * @param padr
     */
    private void loadSimulationMultipleValues(
            Map<Integer, Map<Integer, Double>> valuesMap, String valuesList) {
        for (String record : valuesList.split("_")) {
            String[] parts = record.split(",");
            int id = Integer.parseInt(parts[0]);
